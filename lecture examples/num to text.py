...
OTHERS = ('', 'hundred', 'thousand', 'million', 'billion',
          'trillion', 'quadrillion', 'quintillion',
          'sextillion', 'septillion')

OTHER_INTS = [10, 100] + [1000**n for n in range(1, len(OTHERS))]
...
def num_helper(n: int, index: int) -> str:
    top_half = n // OTHER_INTS[index]
    bottom_half = n % OTHER_INTS[index]
    if bottom_half == 0:
        return TENS[top_half] if index == 0 else num_recurse(top_half) + " " + OTHERS[index]
    return (
        (TENS[top_half] if index == 0 else num_recurse(top_half)) +
        " " + OTHERS[index] + " " + num_recurse(bottom_half)
    )


def get_text_from_num(value: str) -> str:
    try:
        input_integer = int(value)
        sign = 'negative ' if input_integer < 0 else ''
